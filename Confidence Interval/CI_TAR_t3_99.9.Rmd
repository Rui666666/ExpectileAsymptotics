...
TAR with normal and t(3) innovations
```{r}
...
set.seed(101)
#sample_TAR = simnlts(100000000, alpha, thr = 0, sigma = 1) #normal
sample_TAR = simnlts(100000000, alpha, thr = 0, df = df_t) #t distribution
expectile_true = expectile(as.vector(sample_TAR), probs = tau_vec)
time_end = Sys.time()
...
fold = function(k,N,Mrep){
  ...
  cv = cv_vec_0.99[k-1]
  # 0.999 CI
  cv_vec_0.99 = qt(p = 0.001/2, df = c(1:9), lower.tail = FALSE)
  cv = cv_vec_0.99[k-1]
...
xtable= xtable(prob,digits=c(0,0,0,rep(4,9)))
...
xtable= xtable(width,digits=c(0,0,0,rep(4,9)))
print(xtable, include.rownames = FALSE,sanitize.text.function = function(x) {x})
```
